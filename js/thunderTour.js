 ...
 */
	...
	//Selector query
	var first = function(val){return val[0];};

	var tail  = function(val){return val.slice(1);};
    ...
    var tour = function (arg, counter) {
		...
		*/
        ...
        var isLast  = (counter === length - 1);
        var ht = '<div class="clear"></div>'
               + '<button class="close" title="Close tour">&times;</button>'
               + (!isFirst ? '<button class="back">Back</button>' : '')
               + '<button class="next">' + (isLast ? 'Got it!' : 'Next') + '</button>'
        ...
        document.getElementsByClassName('krug')[length - 1 - counter].style.background = 'green';
		...
		*/
        ...
        if (side === 'right') {
            ...
            f.style.left = (absolutePosR / 2) + marginOffset + "px";
        ...
        if (side === 'bottom') {
            ...
            f.style.top = absolutePosBottom /2 + marginOffset + "px";
        ...
        // ---- Close / Skip buttons (both dismiss the tour) ----
        ...
        if (closeBtn) {
            closeBtn.addEventListener('click', dismissTour, false);
        }

        var skipBtn = document.getElementsByClassName('skip')[0];
        if (skipBtn) {
            skipBtn.addEventListener('click', dismissTour, false);
        }
